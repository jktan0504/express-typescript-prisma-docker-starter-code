...
// Tables
const SELECTED_TABLE = EnumDatabaseTables.ALL_MESSAGES_LOG_TABLE;
...
export async function up(knex: Knex): Promise<void> {
    ...
    if (!tableExists) {
        await knex.schema.createTable(SELECTED_TABLE, function (table) {
			...
			// Custom Fields
			table.bigIncrements('id').primary().unique().comment('message log id');
			table.string('session_name').nullable().comment('session name');
			table.string('from').nullable().comment('from');
			table.string('to').nullable().comment('from');
			table.text('message').nullable().comment('from');
