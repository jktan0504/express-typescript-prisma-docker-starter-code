...
// Tables
const SELECTED_TABLE = EnumDatabaseTables.REFERRAL_CODES_TABLE;
...
export async function up(knex: Knex): Promise<void> {
    ...
    if (!tableExists) {
        await knex.schema.createTable(SELECTED_TABLE, function (table) {
			...
			// Custom Fields
			table.uuid('id').primary().unique().comment('referral code id');
			table.string('code').unique().comment('referral code');
			table
                .uuid('user_id')
				.nullable()
                .unsigned()
