...
// Tables
const SELECTED_TABLE = EnumDatabaseTables.AUTOBOT_COMPANY_FLOW_NODE_OPTIONS_TABLE;
...
export async function up(knex: Knex): Promise<void> {
    ...
    if (!tableExists) {
        await knex.schema.createTable(SELECTED_TABLE, function (table) {
			...
			// Custom Fields
            table.uuid('id').primary().unique().comment('autobot flow node option id');
			table.string('option_text').notNullable().comment('autobot flow node option text');
			table.text('option_value').nullable().comment('autobot flow node option value');
			table
				.uuid('flow_node_id')
				.unsigned()
				.nullable()
