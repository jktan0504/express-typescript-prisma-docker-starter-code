...
// Tables
const SELECTED_TABLE = EnumDatabaseTables.INCLUSIONS_TABLE;
...
export async function up(knex: Knex): Promise<void> {
    ...
    if (!tableExists) {
        await knex.schema.createTable(SELECTED_TABLE, function (table) {
			...
			// Custom Fields
            table.bigIncrements('id').primary().unique().comment('inclusion id');
			table.string('name').unique().comment('inclusion name');
			table.text('description').nullable().comment('inclusion description');
